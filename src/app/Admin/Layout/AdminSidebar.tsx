      ...
      <nav className="">
          {
            menuItemDashboard.map(({icon:Icon,label,path})=>(
              <Link
                key={path}
                to={path}
                className={`flex items-center px-6 py-3 text-gray-700 hover:bg-gray-100 ${location.pathname===path ? 'bg-blue-50 text-blue-600 border-r-4 border-blue-600':''}`}
                >
        ...
        {menuItems.map(({ icon: Icon, label, path }) => (
          <Link
            ...
            className={`
              ...
              ${location.pathname === path ? 'bg-blue-50 text-blue-600 border-r-4 border-blue-600' : ''}
            `}
          >
